...
describe("Teste de login", () => {
    test("Erro que deve retornar quando o email fornecido não existe", async () => {
        expect.assertions
        try {
            await userBusinessMock.login("fla@lab.com", "flavio123")
        } catch (error) {
            if (error instanceof CustomError) {
    ...
    test("Erro que deve retornar quando a senha está errada", async () => {
        expect.assertions
        try {
            await userBusinessMock.login("flavio@lab.com", "flavio321")
        } catch (error) {
            if (error instanceof CustomError) {
                expect(error.message).toEqual("Invalid credentials")
                expect(error.statusCode).toEqual(401)
    ...
    test("Sucesso no login e verificação do token de acesso", async () => {
        expect.assertions
        try {
            const result = await userBusinessMock.login("flavio@lab.com", "flavio123")
